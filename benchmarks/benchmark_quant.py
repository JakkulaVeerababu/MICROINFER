"""
MicroInfer - Phase 4: INT8 Quantized Model Benchmarking Harness
Measures TTFT, TPOT, throughput, and VRAM memory footprint for 8-bit quantized weight execution.
"""
...
def run_quant_benchmark(
    ...
    """
    Executes benchmark for Phase 4 INT8 Quantized Generator Engine.
    """
    ...
    print("\n" + "=" * 60)
    print(f"  MICROINFER PHASE 4: INT8 QUANTIZED ENGINE BENCHMARK")
    ...
    _ = quant_generate(model, tokenizer, "Warm up prompt", max_new_tokens=5, temperature=0.0)
    ...
    if torch.cuda.is_available():
        ...
        print(f"Peak VRAM Usage under INT8 Execution: {peak_vram_gb:.2f} GB")

    output_dir = Path(__file__).parent / "results"
    output_dir.mkdir(exist_ok=True, parents=True)
    out_file = output_dir / "phase4_quant.json"
    ...
    print(f"\n[MicroInfer] Phase 4 INT8 metrics saved to '{out_file}'.")
    return export_data
